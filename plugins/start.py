...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To Warrior Video Player.\n\nIts Time To Create Account On Warrior Video Player\n\nNow Send Me Your Business Name Which Show On Website\nEx :- <code>Warrior Units</code></b>")
        if name.text:
...
async def stream_start(client, message):
    ...
    encoded_url = f"{LINK_URL}?Warrior_Player={link}"
